...
## En guerre contre la souris

En tant que développeur adepte du [principe de Pareto](https://fr.wikipedia.org/wiki/Principe_de_Pareto), je cherchais d’abord à remplacer mon éditeur de texte ([VSCodium](https://vscodium.com/)) et mes <abbr title="Environnement de Développement Intégré">EDI</abbr> (Visual Studio et Android Studio, principalement) par un **éditeur modal**. L’exemple le plus connu est [Vim](https://www.vim.org/), dont vous avez certainement entendu parler si vous développez aussi. La particularité des éditeurs modaux est que la navigation s’effectue *entièrement* au clavier. Avant que certain·e·s d’entre vous ne prennent un air horrifié à cette idée, laissez-moi vous rassurer : nul besoin d’apprendre par cœur une centaine de raccourcis clavier pour maîtriser ce genre d’outil.

Comme son nom l’indique, l’éditeur modal comporte plusieurs modes qui ont chacun leur utilité pour manipuler le texte. Chaque mode dispose de ses propres commandes, c’est-à-dire qu’une même touche — par exemple la lettre `i` — n’a pas la même signification en fonction du mode actif. Le mode **normal** est actif par défaut ; c’est grâce à lui que vous pouvez vous déplacer dans le document et effectuer toute sorte d’opérations sur le texte : sélection, copier-coller, chercher-remplacer, etc. Pour commencer à saisir du texte comme dans un éditeur graphique, il suffit de presser la lettre `i` pour activer le mode **insertion**. Votre clavier se comporte à nouveau comme à son habitude — ouf ! De retour au mode normal, ce ne sont pas de bêtes raccourcis que vous utilisez mais un véritable **langage**. On parle souvent de grammaire pour décrire les règles qui régissent les commandes des éditeurs tels que Vim. Dans le cas de ce dernier, chaque commande fonctionne un peu sur le même principe qu’une phrase en français : verbe + objet. Par exemple, pour dire à Vim de supprimer les trois prochains mots à partir du curseur, vous écrivez l’équivalent de « supprime 3 mots », ce qui en langage Vim se traduit par `d3w` (*delete 3 words*). À mesure que l’on apprend de nouveaux verbes et objets, on peut composer de manière très intuitive des commandes complexes qui demanderaient de nombreux *clicks* ou autres raccourcis cryptiques dans un éditeur graphique.

En plus de libérer la main droite de la souris, l’éditeur modal apporte un immense gain de productivité. Toutes les commandes imaginables sont à portée de doigts et si la configuration par défaut ne convient pas, il est possible de la personnaliser dans les moindres détails. Après quelques années d’utilisation, les utilisat·eur·rice·s témoignent souvent que l’édition se fait à la vitesse de la pensée.

{% warning() %}
Tous ces bénéfices ont un coût : celui d’un apprentissage dont la durée en a déjà découragé plus d’un·e. Je vais être honnête avec vous, les premières semaines voire mois passés avec un tel outil sont tout sauf agréables. Il faut se préparer à une nette baisse de sa productivité pour ensuite la voir remonter progressivement, le temps que la mémoire musculaire assimile les mouvements nécessaires.
{% end %}

Finalement, ce n’est pas sur Vim, ni son « successeur » le plus connu, [Neovim](https://neovim.io/), que j’ai jeté mon dévolu, mais sur [Helix](https://helix-editor.com/), un autre éditeur modal relativement jeune. Plusieurs raisons m’ont poussé à choisir Helix plutôt que Vim. J’adorerais les aborder en détail ici, mais je crains que cela ne s’éloigne trop du sujet, raison pour laquelle j’y consacrerai un article dédié.
